...
def paste_roi_imageMaxSize(image_plan, image_validation, ablation_image, tumor_image, flag_source_img):
    ...
    dimension = image_plan.GetDimension() #
    reference_physical_size = np.zeros(dimension)
    reference_direction = np.identity(dimension).flatten()
    reference_size = [512] * dimension
    reference_origin = np.zeros(dimension)
    data = [image_plan, image_validation, ablation_image, tumor_image]
    for img in data:
        reference_physical_size[:] = [(sz - 1) * spc if sz * spc > mx else mx for sz, spc, mx in
                                      zip(img.GetSize(), img.GetSpacing(), reference_physical_size)]
    reference_spacing = [phys_sz / (sz - 1) for sz, phys_sz in zip(reference_size, reference_physical_size)]
    # reference_spacing = [1,1,1]
    reference_image = sitk.Image(reference_size, image_plan.GetPixelIDValue())
    ...
    for img in data:
        ...
        transform.SetTranslation(np.array_equal(img.GetOrigin) - reference_origin)
