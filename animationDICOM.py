...
im2 = plt.imshow(mask_tumor_nda[0,:,:], cmap='RdYlBu', alpha=0.3, interpolation='none',animated=True)
ims = []
...
def updatefig(z):
    ...
    TumorOverlay = mask_tumor_nda[z,:,:].astype(np.float)
...
ani = animation.FuncAnimation(fig, updatefig, frames = np.arange(1, slices), interval=25)
