...
def plotBoxplots(data_distances, tumor_volume_coverage_list, rootdir):
    ...
    plt.ylabel('[mm]', fontsize=24, color='black')
    ...
    ax2.margins(0)
    ax2.scatter(xs, ys, s=300, marker='o', c='steelblue', edgecolors='darkblue')
    plt.setp(ax2.get_xticklabels(), fontsize=24, color='black')
    plt.setp(ax2.get_yticklabels(), fontsize=24, color='black')
    ax2.set_ylim([0, 1.04])
    ax2.set_xlim([0.5, len(ys) + 0.5])
    plt.ylabel('Tumour Volume Coverage Ratio', fontsize=24, color='black')
    plt.setp(ax2.get_yticklabels(), color='k')
    plt.xlabel('Case', fontsize=24, color='black')
    figpath = os.path.join(rootdir, 'DistanceMapsBoxplots_vs_ScatterPlotVolume.png')
    plt.show()
    gh.save(figpath, width=24, height=20)
